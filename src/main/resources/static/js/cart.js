$(function () {
    ...
    function checkCart(id) {
        ...
        var cart_total_price = parseFloat($('#cart_total').html());
    ...
    function reloadCartList(cart_list) {
        ...
        for(var i = 0; i < len; i ++){
            var cartVO = cart_list[i];
            var itemVO = getItem(cartVO.itemId);
            var htmlContent = "";
            htmlContent += "<ul class=\"cart_list_td clearfix\">" +
                "       <li class=\"col01\">";
            if(cartVO.status == 1){
                htmlContent += "<input type=\"checkbox\" name=\"\" id=\"check_"+cartVO.id+"\" data-id=\""+cartVO.id+"\">";
            }else{
                htmlContent += "已失效";
            }
            htmlContent += "</li>" +
                "       <li class=\"col02\"><img src=\""+itemVO.imgUrl+"\"></li>" +
                "       <li class=\"col03\">"+itemVO.title+"<br><em>"+itemVO.price.toFixed(2)+"元</em></li>" +
                "       <li class=\"col04\">1件</li>" +
                "       <li class=\"col05\"  id=\"price_"+cartVO.id+"\" data-price=\""+cartVO.price+"\">"+cartVO.price.toFixed(2)+"元</li>" +
                "       <li class=\"col06\">" ;
            if(cartVO.status == 1){
                htmlContent +=
                    "          <div class=\"num_add\">" +
                    "               <a href=\"#\" class=\"add fl\" id=\"add_"+cartVO.id+"\" data-id=\""+cartVO.id+"\">+</a>" +
                    "               <input type=\"text\" class=\"num_show fl\" value=\""+cartVO.amount+"\" id=\"amount_"+cartVO.id+"\">" +
                    "               <a href=\"#\" class=\"minus fl\" id=\"minus_"+cartVO.id+"\" data-id=\""+cartVO.id+"\">-</a>" +
                    "          </div>";
            }else{
                htmlContent += "已失效";
            }
            htmlContent +=
                "       </li>" +
                "       <li class=\"col07\" id=\"totalPrice_"+cartVO.id+"\">"+cartVO.totalPrice.toFixed(2)+"元</li>" +
                "       <li class=\"col08\"><a href=\"#\" id=\"delete_"+cartVO.id+"\" data-id=\""+cartVO.id+"\">删除</a></li>" +
                "</ul>";
            ...
            $("#add_"+cartVO.id).on("click",function () {
                var id = $(this).data("id");
                var past_amount = parseInt($("#amount_"+id).val());
                if( past_amount< 99 ){
                    var amount = past_amount + 1;
                    $.ajax({
                        type:"POST",
                        ...
                        success:function (data) {
                            if(data.status == "success") {
                                ...
                                var total_price = parseFloat($("#price_"+id).data("price"))*100*amount/100;
