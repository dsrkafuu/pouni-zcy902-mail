    ...
    $.ajax({
        ...
        success:function (data) {
            if(data.status == "success") {
                ...
                alert("获取"+list_title[i-1]+"分类列表失败，原因是"+data.data.errMsg);
                if(i<=5){
                    getData(i+1);
                }
            }
        ...
        error:function (data) {
            alert("获取"+list_title[i-1]+"分类列表失败，原因是"+data.responseText);
            if(i<=5){
                getData(i+1);
            }
        }
