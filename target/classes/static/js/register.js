$(function(){
			...
			$.ajax({
				...
				success:function (data) {
					if(data.status == "success") {
						alert("验证码发送成功，可惜只发给了管理员（我是不会告诉你管理员穷到买不起短信服务的T_T），快找管理员要验证码吧");
					}else {
	...
	$('#register_sub').on("click",function() {
		...
		if(error_name == false && error_password == false && error_check_password == false && error_email == false && error_check == false && error_telphone == false && error_age == false && error_check == false)
		{
